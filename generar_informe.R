#!/usr/bin/env Rscript
# Script para ejecutar todo el proceso de análisis y generar el informe final
...
verificar_estructura <- function() {
  ...
  # Verificar que existan los scripts de análisis
  scripts_requeridos <- c(
    "scripts/01_importar.R",
    "scripts/02_analizar.R",
    "scripts/03_visualizar.R"
  )
  
  scripts_faltantes <- scripts_requeridos[!file.exists(scripts_requeridos)]
  
  if (length(scripts_faltantes) > 0) {
    stop(paste("Faltan scripts necesarios:", 
...
# Función principal
generar_informe_completo <- function() {
  # Registrar hora de inicio
  hora_inicio <- Sys.time()
  cat("=== Iniciando generación de informe:", format(hora_inicio, "%Y-%m-%d %H:%M:%S"), "===\n\n")
  ...
  resultados <- sapply(scripts, ejecutar_script)
  
  # Verificar si todos los scripts se ejecutaron correctamente
  if (all(resultados)) {
    cat("\nTodos los scripts se ejecutaron correctamente\n")
    
    # Renderizar el informe Quarto
    renderizado_exitoso <- renderizar_informe()
    ...
    cat("\nALERTA: Algunos scripts fallaron. Consulta 'logs/errores.log' para detalles.\n")
  }
